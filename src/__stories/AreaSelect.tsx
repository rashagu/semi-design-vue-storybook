...
const AreaSelect = defineComponent<ExampleProps>((props, {slots}) => {
  ...
  return () => (
    <Popover trigger={['click']} placement="bottom" overlayClassName={styles.areaPopover}>
      {{
        content: () => <div style={{width: '28rem',}} class={''}>
          <div class={'flex justify-between items-center pb-3'}>
            <div>
              <RadioGroup onChange={(v: any) => {
                console.debug(v, RadioGroupValue.value)
                RadioGroupValue.value = v.target.value
              }} value={RadioGroupValue.value} size="small">
                <RadioButton value="a">按省份{RadioGroupValue.value}</RadioButton>
                <RadioButton value="b">按城市</RadioButton>
              </RadioGroup>
            </div>
            <div style={{paddingLeft: '.4rem'}}>
              <Input v-model={[searchInputValue.value, 'value']} onChange={(v)=>searchInputChange(v.target.value)} size="small" placeholder="搜索"/>
            </div>
          </div>
          <div class={'flex flex-wrap  pb-2'} style={{height:'4rem'}}>
            {abc.map((item, index) => {
              return <div key={index} style={{padding: '.1rem'}}>
                {RadioGroupValue.value === 'a'?
                  <Button disabled={pinyinArrProvinces.value.indexOf(item) < 0} size={'small'} onClick={() => provincesKey.value = item}
                          type={provincesKey.value === item ? 'primary' : 'default'}>{item}</Button>:
                  <Button disabled={pinyinArrCity.value.indexOf(item) < 0} size={'small'} onClick={() => cityKey.value = item}
                          type={cityKey.value === item ? 'primary' : 'default'}>{item}</Button>}
          ...
          <div style={{height: '20rem', overflow: 'auto'}}>
            {RadioGroupValue.value === 'a' ? provinces.value[provincesKey.value]?.map((item: PcCodeObjectData, index: number) => {
              return <div key={index} class={'flex pb-2'}>
                <div style={{width: '5rem', fontWeight: citySelected.value.provinceCode === item.code?'bolder':'bold',}}>{item.name}</div>
        ...
        default: () => (
          <div class={styles.areaInput} style={props.style}>
            {citySelected.value.name?(citySelected.value.provinceName + '/' + citySelected.value.name):'' || <span class={'ant-select-selection-placeholder'}>{props.placeholder}</span>}
          </div>
        )
      }}
    </Popover>
